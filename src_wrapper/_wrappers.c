 ...
 * using only {int, float, double, char} types (c2py23's current type set).
 *
 * All type-punning casts are valid: char* is used as a generic pointer
 * and cast to the correct type before calling the original function.
...
void refine_assigned(vec ubi[3], vec gv[], int labels[], int label, int *npk, double *drlv2, int ng);
...
void blobproperties(float *data, int *labels, int np, double *results, float omega, int verbose, int ns, int nf);

/* localmaxlabel.c */
int localmaxlabel(float *data, int32_t *labels, uint8_t *wrk, int ns, int nf);
...
void sparse_blob2Dproperties(float *v, uint16_t *i, uint16_t *j, int nnz, int *labels, int npk, double *results);
void sparse_smooth(float *v, uint16_t *i, uint16_t *j, int nnz, float *s);
int sparse_localmaxlabel(float *v, uint16_t *i, uint16_t *j, int nnz, float *MV, int *iMV, int *labels);
...
/* cimaged11utils.c */
void put_incr64(float *data, const int64_t *ind, const float *vals, int boundscheck, int m, int n);
...
/* ---------- score_and_refine: return n, sumdrlv2 via buffer ---------- */
...
{
    int n;
    score_and_refine((vec(*)[3])ubi_ptr, (vec*)gv_ptr, tol, &n,
                     sumdrlv2_buf, ng);
    return n;
}


/* ---------- uint16_t* adapters (uint16 -> char pointer) ---------- */
...
/* ---------- uint8_t* adapters (uint8 -> char pointer) ---------- */
...
/* ---------- int32_t* adapters (int32 -> int pointer) ---------- */
...
/* ---------- int8_t* adapters (int8 -> char pointer) ---------- */

int
clean_mask_wrapper_impl(const char *msk_buf, char *ret_buf, int ns, int nf)
{
    return clean_mask((int8_t *)msk_buf, (int8_t *)ret_buf, ns, nf);
}

int
make_clean_mask_wrapper_impl(const float *img, float cut,
                              const char *msk_buf, char *ret_buf, int ns, int nf)
{
...
/* ---------- uint32_t* adapters (uint32 -> int pointer) ---------- */
...
/* ---------- int16_t* adapter (int16 -> char pointer) ---------- */
...
/* ---------- uint8_t* adapter for splat rgba ---------- */
...
/* ---------- int64_t* adapter (int64 -> char pointer) ---------- */

void
put_incr64_wrapper(float *data, const char *ind_buf, const float *vals,
                    int boundscheck, int m, int n)
{
    put_incr64(data, (const int64_t *)ind_buf, vals, boundscheck, m, n);
}


/* ---------- sparse_image.c uint16_t* adapters ---------- */
...
/* ---------- External declarations for functions used without wrappers ---------- */
/* (declared here to satisfy compiler; defined in respective .c files) */
...
double misori_cubic(double u1[3][3], double u2[3][3]);
double misori_orthorhombic(double u1[3][3], double u2[3][3]);
double misori_tetragonal(double u1[3][3], double u2[3][3]);
double misori_monoclinic(double u1[3][3], double u2[3][3]);
...
void quickorient(double ubi[3][3], double bt[3][3]);
...
int bloboverlaps(int *labels1, int npk1, double *results1, int *labels2, int npk2, double *results2, int ns, int nf);
...
void put_incr32(float *data, const int *ind, const float *vals, int boundscheck, int m, int n);
