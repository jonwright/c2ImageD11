    ...
    const float *gvx, const float *gvy, const float *gvz, double tol, intptr_t ng)
{
    ...
    return n + score_tail_soa_f32(ubi, gvx + k, gvy + k, gvz + k, tol, ng - k);
}

int score_f32_soa_avx2(const double ubi[3][3], const float gv[], double tol, intptr_t ng)
{
    int n;
    OMP_DISPATCH_INT_SOA(score_f32_soa_avx2_kernel, (const double *)ubi, gv, gv + ng, gv + 2*ng, sizeof(float), ng, tol, n);
    return n;
}
