 ...
 *         "sig": "int score_f32_soa_avx2(const double ubi[3][3], const float gv[], double tol, intptr_t ng) -> int",
...
static int score_f32_soa_avx2_kernel(const double ubi[9],
    const float *gvx, const float *gvy, const float *gvz, double tol, intptr_t ng)
{
    ...
    float t2=(float)(tol*tol); __m256 tvec=_mm256_set1_ps(t2); int n=0; intptr_t k;
    for(k=0;k+8<=ng;k+=8){
        __m256 gvx_v=_mm256_loadu_ps(&gvx[k]),gvy_v=_mm256_loadu_ps(&gvy[k]),gvz_v=_mm256_loadu_ps(&gvz[k]);
        __m256 hx=_mm256_fmadd_ps(u00,gvx_v,_mm256_fmadd_ps(u01,gvy_v,_mm256_mul_ps(u02,gvz_v)));
        __m256 hy=_mm256_fmadd_ps(u10,gvx_v,_mm256_fmadd_ps(u11,gvy_v,_mm256_mul_ps(u12,gvz_v)));
        __m256 hz=_mm256_fmadd_ps(u20,gvx_v,_mm256_fmadd_ps(u21,gvy_v,_mm256_mul_ps(u22,gvz_v)));
        ...
        __m256 tx=_mm256_sub_ps(hx,ihx),ty=_mm256_sub_ps(hy,ihy),tz=_mm256_sub_ps(hz,ihz);
        ...
        int mm=_mm256_movemask_ps(mask); if(mm)n+=popcnt32(mm);
    }
    double tol2=tol*tol;
...
int score_f32_soa_avx2(const double ubi[3][3], const float gv[], double tol, intptr_t ng)
{
    ...
    if(ng>10000&&nthr>1){
        ...
        { int tid=omp_get_thread_num(); intptr_t chunk=(ng+nthr-1)/nthr,start=tid*chunk;
          ...
          if(start<ng)n=score_f32_soa_avx2_kernel((const double*)ubi,gvx+start,gvy+start,gvz+start,tol,end-start);}
        return n;}
#endif
    return score_f32_soa_avx2_kernel((const double*)ubi,gvx,gvy,gvz,tol,ng);
}
