#!/usr/bin/env python3
"""Benchmark score() -- all ISA tiers via _rebind_ variant switching.

One .so, one build.  Uses _rebind_<func>() to select the variant
for each tier+layout combo without rebuilding or flag toggling.
"""
...
def _check_pipe_truncator():
    """Error out if stdout is piped through head or tail."""
    if sys.stdout.isatty():
        return
    s = os.fstat(1)
    if not s.st_mode & 0o010000:  # S_IFIFO (not a pipe)
        return
    # find the reader process on the other end of this pipe
    try:
...
# Mapping tier -> variant name suffix for each layout
# The rebind names match what _variants_score() returns
TIER_VARIANTS = {
    "avx512": {
        "AoS_f64": "score_f64_avx512",
        "SoA_f64": "score_f64_soa_avx512",
        "AoS_f32": "score_f32_avx512",
        "SoA_f32": "score_f32_soa_avx512",
    },
    "avx2": {
        "AoS_f64": "score_f64_avx2",
        "SoA_f64": "score_f64_soa_avx2",
        "AoS_f32": "score_f32_avx2",
        "SoA_f32": "score_f32_soa_avx2",
    },
    "baseline": {
        "AoS_f64": "score",
        "SoA_f64": "score_sov_f64",
        "AoS_f32": "score_f32",
        "SoA_f32": "score_sov_f32",
    },
}
...
def main():
    ...
    rebind = getattr(mod, "_rebind_score", None)
    n_cores = 2
    ...
    for ng in args.sizes[:1]:
        ...
        for label, gv in [("AoS_f64", gv_f64), ("SoA_f64", gv_f64.T.copy()),
            ...
            if rebind: rebind(None)  # reset to default
            ...
            for a in sorted(dir(mod)):
                if "_c2py_ol_ptr_score__" in a:
                    ...
                    if struct.unpack_from("Q", buf)[0] > 0:
                        defaults[label] = a.replace("_c2py_ol_ptr_score__", "")
                        break

    # Determine which ISA tiers are actually available on this CPU.
    # If AVX-512 is available the defaults show avx512 names (avx512 implies avx2).
    # If only AVX2 is available the defaults show avx2 names.
