...
int score_f64_soa_avx2(const double ubi[3][3], const double gv[], double tol, intptr_t ng)
{
    ...
    if(ng>10000&&nthr>1){
        ...
        { int tid=omp_get_thread_num(); intptr_t chunk=(ng+nthr-1)/nthr,start=tid*chunk;
          ...
          if(start<ng)n=score_f64_soa_avx2_kernel((const double*)ubi,gvx+start,gvy+start,gvz+start,tol,end-start);}
        return n;}
#endif
    return score_f64_soa_avx2_kernel((const double*)ubi,gvx,gvy,gvz,tol,ng);
}
