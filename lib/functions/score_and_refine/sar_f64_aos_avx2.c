/* sar_f64_aos_avx2.c -- f64 AoS AVX2 intrinsics variant */
/* Scalar-gather approach for AoS: _mm256_set_pd with 4 explicit loads.
 * Simpler than the 3-overlapping-load + shuffle approach, portable,
 * and the 12 scalar loads per 4-gv iteration are dwarfed by 36 FMAs.
 *
 * C2PY_BEGIN
 * {
    ...
    int *__restrict n_out, double *__restrict sumdrlv2_out)
{
    ...
    __m256d H00=_mm256_setzero_pd(),H01=_mm256_setzero_pd(),H02=_mm256_setzero_pd(),
            H10=_mm256_setzero_pd(),H11=_mm256_setzero_pd(),H12=_mm256_setzero_pd(),
            H20=_mm256_setzero_pd(),H21=_mm256_setzero_pd(),H22=_mm256_setzero_pd();
    __m256d R00=_mm256_setzero_pd(),R01=_mm256_setzero_pd(),R02=_mm256_setzero_pd(),
            R10=_mm256_setzero_pd(),R11=_mm256_setzero_pd(),R12=_mm256_setzero_pd(),
            R20=_mm256_setzero_pd(),R21=_mm256_setzero_pd(),R22=_mm256_setzero_pd();
    __m256d s_vec=_mm256_setzero_pd(); int ns=0; intptr_t k;
    for(k=0;k+4<=ng;k+=4){
        ...
        int mm=_mm256_movemask_pd(mask); if(!mm)continue;
        ns+=popcnt32(mm); s_vec=_mm256_add_pd(s_vec,_mm256_and_pd(sumsq,mask));
#define MA(a,v) a=_mm256_add_pd(a,_mm256_and_pd(v,mask))
        MA(H00,_mm256_mul_pd(ihx,ihx));MA(H01,_mm256_mul_pd(ihx,ihy));MA(H02,_mm256_mul_pd(ihx,ihz));
        MA(H10,_mm256_mul_pd(ihy,ihx));MA(H11,_mm256_mul_pd(ihy,ihy));MA(H12,_mm256_mul_pd(ihy,ihz));
        MA(H20,_mm256_mul_pd(ihz,ihx));MA(H21,_mm256_mul_pd(ihz,ihy));MA(H22,_mm256_mul_pd(ihz,ihz));
        MA(R00,_mm256_mul_pd(ihx,gvx));MA(R01,_mm256_mul_pd(ihy,gvx));MA(R02,_mm256_mul_pd(ihz,gvx));
        MA(R10,_mm256_mul_pd(ihx,gvy));MA(R11,_mm256_mul_pd(ihy,gvy));MA(R12,_mm256_mul_pd(ihz,gvy));
        MA(R20,_mm256_mul_pd(ihx,gvz));MA(R21,_mm256_mul_pd(ihy,gvz));MA(R22,_mm256_mul_pd(ihz,gvz));
#undef MA
    }
    H[0]=hsum4(H00);H[1]=hsum4(H01);H[2]=hsum4(H02);H[3]=hsum4(H10);H[4]=hsum4(H11);H[5]=hsum4(H12);
    ...
    *n_out=ns; *sumdrlv2_out=hsum4(s_vec);
    ...
    for(;k<ng;k++){double gx=gv[k*3],gy=gv[k*3+1],gz=gv[k*3+2];
        ...
        double ix=(hx_+magic)-magic,iy=(hy_+magic)-magic,iz=(hz_+magic)-magic;
...
void score_and_refine_f64_avx2(double ubi[3][3], const double gv[], double tol, int *n_arg, double *sumdrlv2_arg, intptr_t ng)
{
    ...
    sar_f64_aos_avx2_kernel((const double*)ubi,gv,tol,ng,(double*)H,(double*)R,&n,&sd);
    if(n>0)sd/=n;
