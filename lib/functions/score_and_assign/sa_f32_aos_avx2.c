/* sa_f32_aos_avx2.c -- f32 AoS AVX2 intrinsics for score_and_assign()
 *
 * C2PY_BEGIN
 * {
 ...
 *         "sig": "int score_and_assign_f32_aos_avx2(double ubi[3][3], const float gv[], double tol, float *drlv2, int *labels, int label, intptr_t ng) -> int",
...
static int
sa_f32_aos_avx2_kernel(const double ubi[9], const float *gv, double tol,
                        float *drlv2, int *labels, int label, intptr_t ng)
{
    ...
    for(;k<ng;k++){
        ...
        float hx_=(float)ubi[0]*gx+(float)ubi[1]*gy+(float)ubi[2]*gz;hx_-=nearbyintf(hx_);
        float hy_=(float)ubi[3]*gx+(float)ubi[4]*gy+(float)ubi[5]*gz;hy_-=nearbyintf(hy_);
        float hz_=(float)ubi[6]*gx+(float)ubi[7]*gy+(float)ubi[8]*gz;hz_-=nearbyintf(hz_);
                                   ...
                                   float *drlv2, int *labels, int label, intptr_t ng)
{
    ...
    if(ng>10000&&nthr>1){
        ...
        { int tid=omp_get_thread_num(); intptr_t chunk=(ng+nthr-1)/nthr,start=tid*chunk;
          ...
          if(start<ng)n=sa_f32_aos_avx2_kernel((const double*)ubi,gv+start*3,tol,drlv2+start,labels+start,label,end-start);}
        return n;}
#endif
    return sa_f32_aos_avx2_kernel((const double*)ubi,gv,tol,drlv2,labels,label,ng);
}
