#!/usr/bin/env python3
"""Benchmark score_and_assign -- prints f2py first, then streams each measurement.

Detects the dispatched variant per layout/type combo without rebuilds.
"""
...
def main():
    ...
    print()
    header = "{:>8s}  {:>8s}  {:>9s}  {:>9s}  {:>6s}  {:>6s}  {:>4s}  {:>8s}  {:s}".format(
    ...
    for ng in args.sizes:
        ...
        for label, gv, dv in [
            ...
            dv3 = dv.copy(); lb3 = lb.copy()
            t0 = time.perf_counter()
            for i in range(nc): fn(ubis[i % N_UBIS].copy(), gv, tol, dv3, lb3, 1)
            t1 = time.perf_counter()
            ...
            print("{:>8d}  {:>8s}  {:>9.0f}  {:>9.0f}  {:>5.1f}x  {:>5.1f}x  {:>4d}  {:>8s}  {:s}".format(
                ng, label, thr_1t, thr_nt, v1, vn, n_cores, shape, variant), flush=True)
