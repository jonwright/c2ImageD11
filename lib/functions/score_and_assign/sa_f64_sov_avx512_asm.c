 ...
 *         "sig": "int score_and_assign_f64_sov_avx512_asm(double ubi[3][3], const double gv[], double tol, double *drlv2, int *labels, int label, intptr_t ng) -> int",
...
extern int sa_inner_f64_sov_avx512(const double ubi[9], const double *gvx, const double *gvy,
                         const double *gvz, double tol, double *drlv2, int *labels,
                         int label, intptr_t ng);

int score_and_assign_f64_sov_avx512_asm(double ubi[3][3], const double gv[], double tol,
                  double *drlv2, int *labels, int label, intptr_t ng)
{
    ...
    if (ng > 10000 && nthr > 1) {
        #pragma omp parallel reduction(+:n)
        {
            ...
            if (start < ng)
                n = sa_inner_f64_sov_avx512((const double*)ubi, gvx + start, gvy + start, gvz + start,
                                  tol, drlv2 + start, labels + start, label, end - start);
        }
        return n;
    }
#endif
    return sa_inner_f64_sov_avx512((const double*)ubi, gvx, gvy, gvz, tol, drlv2, labels, label, ng);
}
#else
extern int score_and_assign_f64_sov_avx512(double ubi[3][3], const double gv[],
    double tol, double *drlv2, int *labels, int label, intptr_t ng);

int score_and_assign_f64_sov_avx512_asm(double ubi[3][3], const double gv[],
    double tol, double *drlv2, int *labels, int label, intptr_t ng)
{
    return score_and_assign_f64_sov_avx512(ubi, gv, tol, drlv2, labels, label, ng);
}
#endif
