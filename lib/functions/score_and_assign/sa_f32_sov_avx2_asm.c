 ...
 *         "sig": "int score_and_assign_f32_sov_avx2_asm(double ubi[3][3], const float gv[], double tol, float *drlv2, int *labels, int label, intptr_t ng) -> int",
...
extern int sa_inner_f32_sov_avx2(const double ubi[9], const float *gvx, const float *gvy,
                         const float *gvz, double tol, float *drlv2, int *labels,
                         int label, intptr_t ng);

int score_and_assign_f32_sov_avx2_asm(double ubi[3][3], const float gv[], double tol,
                  float *drlv2, int *labels, int label, intptr_t ng)
{
    ...
    if (ng > 10000 && nthr > 1) {
        #pragma omp parallel reduction(+:n)
        {
            ...
            if (start < ng)
                n = sa_inner_f32_sov_avx2((const double*)ubi, gvx + start, gvy + start, gvz + start,
                                  tol, drlv2 + start, labels + start, label, end - start);
        }
        return n;
    }
#endif
    return sa_inner_f32_sov_avx2((const double*)ubi, gvx, gvy, gvz, tol, drlv2, labels, label, ng);
}
#else
extern int score_and_assign_f32_sov_avx2(double ubi[3][3], const float gv[],
    double tol, float *drlv2, int *labels, int label, intptr_t ng);

int score_and_assign_f32_sov_avx2_asm(double ubi[3][3], const float gv[],
    double tol, float *drlv2, int *labels, int label, intptr_t ng)
{
    return score_and_assign_f32_sov_avx2(ubi, gv, tol, drlv2, labels, label, ng);
}
#endif
