 ...
 *     },
 *     "checks": ["xlylzl.ndim == 2", "xlylzl.shape[1] == 3",
 *         "omega.n == xlylzl.shape[0]",
 *         "t.n == 3",
 *         "gv.ndim == 2", "gv.shape[0] == xlylzl.shape[0]", "gv.shape[1] == 3"],
 *     "gil_release": true,
 *     "c_overloads": [{
...
#pragma omp parallel for if(n > 5000) private(so, co, u, o, d, modyz, ds, v, k)
    for (i = 0; i < n; i++) {
         ...
         *   let R = d[1]^2 + d[2]^2,   N = |d| = sqrt(d[0]^2 + R)
         *
         *   ds * (d[0]/N - 1)
         *     = ds * (d[0] - N) / N
         ...
         *     = ds * (-R) / (N * (d[0] + N))
         *     = -ds * R / (N * (d[0] + N))
         *     = -ds * R * modyz / (d[0] + N)           since modyz = 1/N
