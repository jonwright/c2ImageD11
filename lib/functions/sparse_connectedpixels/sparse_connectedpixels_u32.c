...
/* Instantiate for uint32_t */
#define T_val uint32_t
#define SPARSE_CONNECTEDPIXELS_IMPL_NAME sparse_connectedpixels_impl_u32
#include "sparse_connectedpixels.hpp"
#undef SPARSE_CONNECTEDPIXELS_IMPL_NAME
#undef T_val

int sparse_connectedpixels_u32(uint32_t *restrict v, uint16_t *restrict i,
                               uint16_t *restrict j, intptr_t nnz,
                               uint32_t threshold,
                               int32_t *restrict labels) {
    return sparse_connectedpixels_impl_u32(v, i, j, nnz, threshold, labels);
}
