...
void compute_geometry(const double xlylzl[], const double omega[], double omegasign,
...
#pragma omp parallel for if(n > 5000) private(so, co, u, o, d, modyz, ds, v, k)
    for (i = 0; i < n; i++) {
        ...
        d[0] = xlylzl[i*3+0] - o[0]; d[1] = xlylzl[i*3+1] - o[1]; d[2] = xlylzl[i*3+2] - o[2];
        {
            ...
            modyz = 1. / norm;
            // two theta
            out[i*6+0] = DEG * atan2(sqrt(R), d[0]);
            //     ! k-vector
            ds = 1. / wvln;
            /*
        ...
        // eta
        out[i*6+1] = DEG * atan2(-d[1], d[2]);
        // dstar
        out[i*6+2] = sqrt(k[0] * k[0] + k[1] * k[1] + k[2] * k[2]);
        // g-vector
        matTvec(mat, k, v);
