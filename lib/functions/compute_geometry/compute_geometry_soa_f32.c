...
void compute_geometry_soa_f32(const float xlylzl[], const float omega[],
    ...
    intptr_t i;
    const float *xl = xlylzl, *yl = xlylzl + n, *zl = xlylzl + 2*n;
    float *tth = out, *eta = out + n, *ds_out = out + 2*n;
    float *gx = out + 3*n, *gy = out + 4*n, *gz = out + 5*n;
...
#pragma omp parallel for if(n > 5000) private(so, co, u, o, d, modyz, ds, v, k)
    for (i = 0; i < n; i++) {
        ...
        { float R = d[1]*d[1] + d[2]*d[2];
          ...
          tth[i] = (float)(DEG * atan2f(sqrtf(R), d[0]));
          ds = (float)(1. / wvln);
          k[0] = -ds * R * modyz / (d[0] + 1.f/modyz);
          k[1] = ds * d[1] * modyz;
          k[2] = ds * d[2] * modyz;
        }
        eta[i] = (float)(DEG * atan2f(-d[1], d[2]));
        ds_out[i] = sqrtf(k[0]*k[0] + k[1]*k[1] + k[2]*k[2]);
        matTvec(mat, k, v);
        gx[i] =  co * v[0] + so * v[1];
        gy[i] = -so * v[0] + co * v[1];
        gz[i] = v[2];
