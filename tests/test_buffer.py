...
class TestBlobPropertyConstants:
    """Blob property enum constants must be present on import."""
    ...
    def test_all_present(self):
        for name, expected_value in self.CONSTANTS:
            assert hasattr(ci, name), \
                "missing constant %s" % name
            assert getattr(ci, name) == expected_value, \
                "constant %s expected %d, got %d" % (
                    name, expected_value, getattr(ci, name))
